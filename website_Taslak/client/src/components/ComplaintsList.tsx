  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-semibold text-gray-800">En Çok Şikayetler</h2>
        <span className="text-sm text-gray-500">Top 10</span>
      </div>
      <div className="space-y-3">
        {mockAnalysisData.top_complaints.map((complaint, index) => (
          <div key={index} className="flex items-start p-3 bg-red-50 rounded-lg border border-red-100">
            ...
            <span className="text-gray-700 text-sm">{complaint}</span>
          </div>
        ))}
      </div>
    </div>
